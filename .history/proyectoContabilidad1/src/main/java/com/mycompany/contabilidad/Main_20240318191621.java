                ...
                switch (argumentos[0]) { // Correct syntax with colon (:)
                    case "ADDCLIENT":
                        if (i.checkArgs(argumentos, 7)) {
                            ...
                            String lastName = argumentos[2];
                            String dni = argumentos[3];
                            String address = argumentos[4];
                            String telephone = argumentos[5];
                            String email = argumentos[6];
                            if (i.validarDni(dni)) {
                                if (i.isInt(telephone)) {
                                    Cliente cliente = new Cliente(name, lastName, address, Integer.parseInt(telephone), dni, email);
                                    clientes.add(cliente);
                                    f.writeCliente(cliente);
                                    System.out.println("El cliente " + name + " " + lastName + " se ha registrado correctamente.");
                                } else {
                                    System.out.println("El teléfono debe de ser un número.");
                                }
                            }
                        ...
                        if (i.checkArgs(argumentos, 1)) {
                            if (!facturas.isEmpty()) {
                                ...
                                for (Factura factura : facturas) {
                                    if (!factura.isPayed()) {
                                        contador++;
                                        System.out.println("Factura: " + factura.getId() + "\tCoste neto: " + factura.getId());
                                    }
                                } if (contador > 0) {
