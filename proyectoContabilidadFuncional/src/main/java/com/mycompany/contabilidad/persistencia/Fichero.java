...
public class Fichero {
    ...
    public void writeCliente(Cliente c) {
        ...
        try {
            ...
            bw.write(c.getName() + "-" + c.getLastname() + "-" + c.getAddress() + "-" + String.valueOf(c.getTelephone()) + "-" + c.getNif() + "-" + c.getEmail());
            bw.newLine();
            bw.close();
    ...
    public void writeFactura(Factura f) {
        ...
        try {
            ...
            String pagado = "true";
            String coste = String.valueOf(f.getBruto());
            bw.write(f.getId() + "-" + cliente.getNif() + "-" + pagado + "-" + coste);
            bw.newLine();
            bw.close();
        } catch (IOException ex) {
            System.out.println(ex.getMessage());
    ...
    public void sobrescribirFactura(ArrayList<Factura> facturas) {
        if (!folderFile.exists()) {
            folderFile.mkdir();
        }
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(pathFacturas, false));
            for (Factura factura : facturas) {
                Cliente cliente = factura.getCliente();
                if (factura.isPayed()) {
                    String pagado = "true";
                    String coste = String.valueOf(factura.getBruto());
                    bw.write(factura.getId() + "-" + cliente.getNif() + "-" + pagado + "-" + coste);
                    bw.newLine();
                }
                else {
                    String pagado = "false";
